...
const HistoryPage = () => {
  ...
  const handleDelete = async (item: HISTORY) => {
    ...
    try {
      ...
      // delete from DB
      await db.delete(AiOutput).where(eq(AiOutput.id, item.id));
      // optimistic UI update
      setHistory(prev => prev.filter(h => h.id !== item.id));
      // decrement usage by response length (ensure non-negative)
      const decrement = (item.aiResponse || '').length;
      setTotalUsage(Math.max(0, (totalUsage || 0) - decrement));
    } catch (e) {
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-6 gap-4 text-center font-semibold bg-gray-200 p-2">
        <div>Template</div>
      ...
      {history.map((item) => (
        <div key={item.id} className="grid grid-cols-1 md:grid-cols-6 gap-6 p-3 border-b">
          ...
          <div className='md:mx-14'>
            <Button variant="destructive" className='flex gap-2' disabled={!!deleting[item.id]} onClick={() => handleDelete(item)}>
              <Trash2 className='w-4 h-4' /> {deleting[item.id] ? 'Deleting...' : 'Delete'}
            </Button>
          </div>
        </div>
      ))}
